        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border-l-4 border-yellow-500">
            <h3 class="text-black text-[10px] font-bold uppercase tracking-wider mb-1">Storage Drive</h3>
             <div class="flex items-end justify-between">
                <p class="text-xl font-bold text-black">{{ number_format($stats['storage']['usage'] / 1024 / 1024 / 1024, 2) }} GB</p>
                <span class="text-[10px] text-black font-bold">{{ $stats['storage']['limit'] > 0 ? round(($stats['storage']['usage'] / $stats['storage']['limit']) * 100, 1) : 0 }}%</span>
            </div>
             <div class="w-full bg-gray-100 rounded-full h-1.5 mt-2">
                <div class="bg-yellow-500 h-1.5 rounded-full transition-all duration-500" style="width: {{ $stats['storage']['limit'] > 0 ? ($stats['storage']['usage'] / $stats['storage']['limit']) * 100 : 0 }}%"></div>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="space-y-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 bg-primary text-white flex justify-between items-center">
                ...
                <span class="text-[10px] uppercase font-bold tracking-widest bg-white/20 px-2 py-0.5 rounded">Terbaru</span>
            </div>
            <div class="p-6">
                @forelse($infos as $info)
                <div class="group border-b border-black/5 pb-6 mb-6 last:mb-0 last:border-0 last:pb-0">
                    <h4 class="text-md font-bold text-black border-l-4 border-primary pl-3 mb-2 group-hover:text-primary-hover transition-colors">{{ $info->title }}</h4>
                    ...
                    <div class="text-[10px] uppercase font-bold text-black/60 mt-4 flex items-center gap-4">
                        <span class="flex items-center gap-1.5"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>{{ $info->creator->name }}</span>
                    </div>
                </div>
                @empty
    ...
    <div class="lg:col-span-2 space-y-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-4 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
                ...
                <a href="{{ route('events.index') }}" class="text-[10px] font-bold text-primary uppercase hover:underline">Full View</a>
            </div>
            ...
            <div class="px-6 pb-6 pt-4 border-t border-gray-100 bg-gray-50/30">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center gap-4 p-3 rounded-xl bg-white border border-black/5 hover:border-primary/50 transition-all shadow-sm group">
                        <div class="w-12 h-12 rounded-lg bg-primary text-white flex flex-col items-center justify-center flex-shrink-0 shadow-md transform group-hover:scale-105 transition-transform">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-[10px] text-black font-extrabold uppercase tracking-tighter opacity-70">{{ $event->start->format('H:i') }} • {{ $event->type }}</p>
                        </div>
                    </div>
                    @empty
                    <p class="text-black/40 text-[11px] py-4 italic font-extrabold text-center col-span-2">Tidak ada agenda minggu ini.</p>
                    @endforelse
                </div>
            </div>
        </div>
